...
describe('formatarData', () => {
  ...
  test('deve formatar data com hora corretamente', () => {
    expect(formatadores.formatarData('2023-01-01T15:30:00', true)).toBe(
      '01/01/2023 15:30',
    );
  });
...
describe('capitalizarPalavras', () => {
  test('deve capitalizar cada palavra', () => {
    expect(formatadores.capitalizarPalavras('teste de palavras')).toBe(
      'Teste De Palavras',
    );
  });
...
describe('limparEspacos', () => {
  test('deve remover espaços extras', () => {
    expect(formatadores.limparEspacos('  texto  com   espaços ')).toBe(
      'texto com espaços',
    );
  });
...
describe('formatarTelefone', () => {
  test('deve formatar telefone corretamente', () => {
    expect(formatadores.formatarTelefone('11987654321')).toBe(
      '(11) 98765-4321',
    );
    expect(formatadores.formatarTelefone('(11)98765-4321')).toBe(
      '(11) 98765-4321',
    );
  });
...
describe('paraCamelCase', () => {
  test('deve converter para camelCase', () => {
    expect(formatadores.paraCamelCase('teste de camel case')).toBe(
      'testeDeCamelCase',
    );
    expect(formatadores.paraCamelCase('Teste-De_camel.case')).toBe(
      'testeDeCamelCase',
    );
  });
